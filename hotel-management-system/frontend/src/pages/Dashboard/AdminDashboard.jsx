...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const unsubReservation = onEvent('reservation-updated', () => {
      fetchDashboardData();
    });
    const unsubFeedback = onEvent('feedback-updated', () => {
      fetchDashboardData();
    });
    const unsubHotel = onEvent('hotel-updated', () => {
      fetchDashboardData();
    });
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (data.success) {
        setStats(data.stats);
        setRecentBookings(data.recentBookings || data.recentActivities || []);
      ...
      // Only show toast on first load, not on socket refreshes
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <Row className="g-4">
        ...
        <Col md={4}><Card className="modern-card h-100"><Card.Header><h5 className="mb-0">Quick Stats</h5></Card.Header><Card.Body>
          {[{icon:<FaTools/>,label:'Maintenance',value:stats?.maintenanceRooms||0,color:'#C1121F'},{icon:<FaBroom/>,label:'Cleaning',value:stats?.cleaningRooms||0,color:'#E9C46A'},{icon:<FaCalendarCheck/>,label:'Pending Reservations',value:stats?.pendingReservations||0,color:'#457B9D'},{icon:<FaDollarSign/>,label:'Monthly Revenue',value:`$${(stats?.monthlyRevenue||0).toLocaleString()}`,color:'#2D6A4F'},{icon:<FaUsers/>,label:'Guest Feedback',value:`${stats?.totalFeedback||0} (${stats?.averageRating||0}★)`,color:'#0B1D3A'}].map((item,i)=>(
      ...
      <Row className="g-4 mt-1">
        <Col md={12}>
          <Card className="modern-card">
            <Card.Header><h5 className="mb-0">Latest Guest Feedback</h5></Card.Header>
            <Card.Body>
              {recentFeedback.length > 0 ? recentFeedback.map((fb) => (
                <div key={fb._id} className="d-flex justify-content-between align-items-start py-2 border-bottom">
                  <div>
                    <strong>{fb.guest ? `${fb.guest.firstName} ${fb.guest.lastName}` : 'Guest'}</strong>
                    <p className="mb-1 text-muted">{fb.comment || fb.title || 'No comment provided'}</p>
                  </div>
                  <Badge bg="primary">{fb.rating}★</Badge>
                </div>
